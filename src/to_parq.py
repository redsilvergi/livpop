...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 14 16:35:36 2025
...
class XpopSeoul:
    ...
    def combine_datas0(self, df, conn, file_name, file_index, len_csv_files):
        ...
        try:
            processed_dir = os.path.join(self.extract_dir, "processed")
            os.makedirs(processed_dir, exist_ok=True)
            output_path = os.path.join(processed_dir, f"{file_name}.parquet")
            dftmp = pd.DataFrame(dict_list)
            dftmp.to_parquet(output_path, index=False, compression="snappy")
            print(f"File saved successfully to {output_path}")
            # print(f"dict_list: {dict_list}")
            print(f"{file_index}/{len_csv_files} - {file_name} - saved")
...
실제 동작
"""
for month in ["202311", "202310", "202309", "202308", "202307", "202306", "202305"]:
    XpopSeoul(
        work_dir="/Users/eungi/Desktop/eungi_dt/mim/livpop/src/data/",
        extract_dir=f"/Users/eungi/Desktop/eungi_dt/mim/livpop/src/data/local-{month}",
    ).make_combine()
